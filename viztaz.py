...
ggdf = geopandas.read_file(url)

url = r".\shapefiles\blocks20a.shp"
bldf = geopandas.read_file(url)

url = r".\shapefiles\CTPS_TDM23_TAZ_2017g_v202303.shp"
ogdf = geopandas.read_file(url)


oldtaz = [5]

def before_after(oldtaz):
    """
    oldtaz,newtaz = [5],[6,7]
    oldtaz -- old taz
    newtaz -- new taz
    """
    oldtarget = ogdf.query(f"taz_id in {oldtaz}")
    ...
    bldf_filtered = bldf[bldf.intersects(oldtarget_buffer.unary_union)]
    ogdf_filtered = ggdf[ggdf.intersects(oldtarget_buffer.unary_union)]
    ...
    # to__taz = to__taz.to_crs(epsg=3857)
    ...
    # bldf_filtered.plot(color="#367BCA", alha=opacity, ax=ax)
    # Plot the block borders with a dashed black line
    ...
    # - 'CartoDB.Positron' for a light theme
    # - 'CartoDB.DarkMatter' for a dark theme
    # - 'OpenStreetMap.Mapnik' for standard OpenStreetMap style
    # - 'Stamen.Terrain' for terrain visualization
    ctx.add_basemap(ax, 
...
import pandas as pd
dfix = pd.read_excel('TAZ delineation notes.xlsx')
