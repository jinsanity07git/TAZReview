...
Implements:
- 4 Maps (top-left = Old TAZ (blue), top-right = New TAZ (red; blocks not selectable),
  bottom-left = Combined with blocks (translucent yellow) behind new TAZ behind old TAZ,
  bottom-right = blocks (also translucent yellow, black dotted border).
- 2 Tables on the right (~1/4 width).
- 2-Row top layout:
   Row 1 => [Enter Old TAZ + text] (left), [Currently Searching TAZ: #] (middle),
            [Selected Map Background: + dropdown] (right)
   Row 2 => [Search TAZ] (left), [Match 1st Panel Zoom] (right)
- If TAZ invalid => label is bold/italic/red "[TAZ Not Found]",
  else bold/italic/black TAZ ID.
- "Sum" row always visible at bottom of each table, bolded via HTMLTemplateFormatter.
- Pressing Enter triggers the same search logic as "Search TAZ".
- All 4 maps unify to the bounding box of the Old TAZ search result.
"""
...
# Convert all to EPSG:3857 so they align with default tile providers
...
def split_multipolygons_to_cds(gdf, id_field, ensure_cols=None):
    """
    Each sub-polygon => separate row => no KeyError on selection.
    """
...
def add_sum_row(d, colnames):
    ...
    if not d or not d['id']:
        # If there's no main data, we still add a 'Sum' row with 0's
        d['id'] = []
        for c in colnames:
            d[c] = []
...
# -----------------------------------------------------------------------------
# 5. Figures (sizing_mode="scale_both", match_aspect=True)
# -----------------------------------------------------------------------------
...
p_blocks = figure(
    title="4) Blocks (selectable, yellow fill)",
    match_aspect=True,
    tools=TOOLS + ",tap", active_scroll='wheel_zoom',
    x_axis_type="mercator", y_axis_type="mercator",
    sizing_mode="scale_both"
...
# Panel #2 => New TAZ => red, blocks => black dotted, not selectable
...
# Panel #3 => Combined => user wants new TAZ on bottom, old TAZ above that, blocks on top
# So we add patches in the order: new => old => blocks
p_combined.patches(
    xs="xs", ys="ys",
...
# Panel #4 => blocks => same color as bottom-left => yellow fill
p_blocks.patches(
    xs="xs", ys="ys",
    source=blocks_source,
    fill_color="yellow", fill_alpha=0.3,
    line_color="black", line_width=2, line_dash='dotted',
    # user wants them to keep black dashed border on selection => so no change
    selection_fill_color="yellow", selection_fill_alpha=0.3,
    selection_line_color="black", selection_line_dash='dotted',
    nonselection_fill_alpha=0.3,
    nonselection_line_color="black",
    nonselection_line_dash='dotted'
)

# -----------------------------------------------------------------------------
# 7. Tables with a "Sum" row always, bold formatting
# -----------------------------------------------------------------------------
from bokeh.models.widgets.tables import HTMLTemplateFormatter
...
tile_label   = Div(text="<b>Selected Map Background:</b>", width=160)
tile_select  = Select(
    title="",
...
def run_search():
    ...
    if o is None or o.empty:
        search_label.text = "<b>Currently Searching TAZ: </b><b><i style='color:red'>[TAZ Not Found]</i></b>"
        return
    # valid TAZ => show ID in black bold italic
    ...
    # also match p_new, p_combined, p_blocks
...
def on_text_input_change(attr, old, new):
    """Pressing Enter or losing focus => run_search."""
    run_search()
...
def on_tile_change(attr, old, new):
    ...
    for f in [p_old, p_new, p_combined, p_blocks]:
        old_tile = tile_map.get(f)
        if old_tile and old_tile in f.renderers:
            f.renderers.remove(old_tile)
        new_t = f.add_tile(provider)
        tile_map[f] = new_t
...
# 9. Layout
# -----------------------------------------------------------------------------
# Row 1 => [Enter Old TAZ ID (left), Searching TAZ (center), Selected Map BG (right)]
row1_left = row(label_taz, text_input, Spacer(width=30))
row1_middle = row(search_label, sizing_mode="fixed")
row1_right = row(tile_label, tile_select)

row1 = row(row1_left, Spacer(width=50), row1_middle, Spacer(width=50), row1_right, sizing_mode="stretch_width")

# Row 2 => [Search TAZ (left), Match Zoom (right)]
row2_left  = row(search_button, width=180)
row2_right = row(match_zoom_btn)
row2 = row(row2_left, Spacer(width=300), row2_right, sizing_mode="stretch_width")

# 2×2 maps => left side ~ 3/4
top_maps = row(p_old, p_new, sizing_mode="scale_both")
bot_maps = row(p_combined, p_blocks, sizing_mode="scale_both")
maps_col = column(top_maps, bot_maps, sizing_mode="scale_both")

# Right side => ~1/4 => 2 tables stacked
...
tables_col = column(top_table, bot_table, width=350, sizing_mode="scale_both")

# main row => ~3/4 maps + ~1/4 tables
...
curdoc().title = "4-Panel TAZ (3/4 + 1/4), Two-Row Top, Always-Visible Sum, etc."
